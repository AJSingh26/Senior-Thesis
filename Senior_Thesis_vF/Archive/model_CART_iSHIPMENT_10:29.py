...
model.CHOLD = Param(default=50, mutable=True) # Cost per queued therapy per period at a manufacturing site (tunable)
...
model.Q = Var(model.m, model.t, within=NonNegativeReals) # Queue length (WIP waiting to start processing) at each manufacturing site m and time t
...
# OBJECTIVE FUNCTION
def obj_rule(model):
    # Manufacturing, transport, and per-therapy material+QC costs (as you had)
    base_cost = (
        sum(model.CTM[p] for p in model.p) +
        sum(model.TTC[p] for p in model.p) +
        (model.C_material + model.CQC) * len(model.p)
    )
    # New: WIP holding cost at manufacturing sites (queue cost)
    wip_cost = model.CHOLD * sum(model.Q[m, t] for m in model.m for t in model.t)
    return base_cost + wip_cost
...
# --- Queue balance at manufacturing sites ---
# For t = 1, start empty (or change RHS if you want an initial backlog)
def Q_init_rule(model, m):
    return model.Q[m, 1] == 0
model.Q_init = Constraint(model.m, rule=Q_init_rule)

# For t >= 2: Q[m,t] = Q[m,t-1] + arrivals_to_m_at_t - starts_at_m_at_t
def Q_bal_rule(model, m, t):
    if t == 1:
        return Constraint.Skip
    ...
    return model.Q[m, t] == model.Q[m, t-1] + arrivals_t - starts_t

model.Q_balance = Constraint(model.m, model.t, rule=Q_bal_rule)
...
for m in instance.m:
    for t in instance.t:
        qval = value(instance.Q[m, t])
        if qval > 1e-6:
            print(f"WIP queue at MS site {m} at time t{t} = {qval:.0f}")
