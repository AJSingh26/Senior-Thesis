...
model.HQ = Param(within=NonNegativeReals, initialize=1000.0, mutable=True) # Holding cost per queued therapy per time unit (set to >0 to penalize waiting).
...
model.Q = Var(model.m, model.t, within=NonNegativeReals) # Queue length at each manufacturing site m at the start of time t



# OBJECTIVE FUNCTION
def obj_rule(model):
    return sum( model.CTM[p]for p in model.p )+ sum( model.TTC[p] for p in model.p ) + (model.C_material + model.CQC)* len(model.p) + sum(model.Q[m,t] for m in model.m for t in model.t)*model.HQ
...
# Cumulative precedence: cumulative starts up to t cannot exceed cumulative arrivals up to t
#def MSB5_rule(model, p, m, t):
#    return sum(model.INM[p,m,tt] for tt in model.tt if tt <= t) <= \
#           sum(model.LSA[p,c,m,j,tt] for c in model.c for j in model.j for tt in model.tt if tt <= t)
#model.MSB5 = Constraint(model.p, model.m, model.t, rule=MSB5_rule)


#------------------------------------------------------------
# --- 1) Align LS→MS arrivals with departures using TT1 (no new params needed) ---
def LS_to_MS_alignment_rule(model, p, c, m, j, t):
    # Any LS departure tt that lands exactly at t at the MS?
    deps = [tt for tt in model.t if tt + model.TT1[j] == t]
    if deps:
        # Arrival at time t equals departures that land at t
        return model.LSA[p, c, m, j, t] == sum(model.LSR[p, c, m, j, tt] for tt in deps)
    else:
        # No departure can land at t ⇒ no arrival at t
        return model.LSA[p, c, m, j, t] == 0
model.LS_to_MS_align = Constraint(model.p, model.c, model.m, model.j, model.t, rule=LS_to_MS_alignment_rule)

# --- 2) Gate manufacturing starts by *arrivals* (stronger than by collection alone) ---
# Cumulative precedence: cumulative INM up to t cannot exceed cumulative LSA up to t
def MSB5_rule(model, p, m, t):
    return sum(model.INM[p, m, tt] for tt in model.t if tt <= t) <= \
           sum(model.LSA[p, c, m, j, tt] for c in model.c for j in model.j for tt in model.t if tt <= t)
model.MSB5 = Constraint(model.p, model.m, model.t, rule=MSB5_rule)

# --- 3) Align MS→Hospital arrivals with MS departures using TT3 ---
# If you log *final therapy delivered* as FTD, align that to MSO with TT3:
def MS_to_H_alignment_rule(model, p, m, h, j, t):
    deps = [tt for tt in model.t if tt + model.TT3[j] == t]
    if deps:
        return model.FTD[p, m, h, j, t] == sum(model.MSO[p, m, h, j, tt] for tt in deps)
    else:
        return model.FTD[p, m, h, j, t] == 0
model.MS_to_H_align = Constraint(model.p, model.m, model.h, model.j, model.t, rule=MS_to_H_alignment_rule)

# (Optional) If you also keep INH as a separate "arrived at hospital" variable,
# tie it to FTD one-for-one so prints stay consistent:
def Hospital_arrival_consistency_rule(model, p, h, t):
    # Sum across m,j in case you allow multiple routes/modes
    return sum(model.INH[p, h, t] for _dummy in [0]) == \
           sum(model.FTD[p, m, h, j, t] for m in model.m for j in model.j)
model.HospArriveConsistent = Constraint(model.p, model.h, model.t, rule=Hospital_arrival_consistency_rule)

#------------------------------------------------------------

# Place these in the same block where you define MSB/flow constraints, right after MSB5
##REMOVE THESE TWO IF YOU GET SAME RESULTS

# 1) For every arrival leg to a manufacturing site: block if site not open
def LSA_site_open_rule(model, p, c, m, j, t):
    return model.LSA[p, c, m, j, t] <= model.E1[m]
model.LSA_SITE_OPEN = Constraint(model.p, model.c, model.m, model.j, model.t, rule=LSA_site_open_rule)

# 2) For manufacturing starts: block if site not open
def INM_site_open_rule(model, p, m, t):
    return model.INM[p, m, t] <= model.E1[m]
model.INM_SITE_OPEN = Constraint(model.p, model.m, model.t, rule=INM_site_open_rule)

# Each therapy starts manufacturing exactly once (across all sites and times)
def START_ONCE_rule(model, p):
    return sum(model.INM[p,m,t] for m in model.m for t in model.t) == 1
model.START_ONCE = Constraint(model.p, rule=START_ONCE_rule)

# --- Queue balance at each manufacturing site ---
def QBAL_rule(model, m, t):
    arrivals_t = sum(model.LSA[p,c,m,j,t] for p in model.p for c in model.c for j in model.j)
    starts_t   = sum(model.INM[p,m,t]     for p in model.p)

    # Use the very first time index in model.t as the base period
    if t == model.t.first():
        # Assume no initial queue (set to a Param if you want a nonzero initial backlog)
        return model.Q[m,t] == arrivals_t - starts_t
    else:
        prev_t = t - 1
        return model.Q[m,t] == model.Q[m,prev_t] + arrivals_t - starts_t

model.QBAL = Constraint(model.m, model.t, rule=QBAL_rule)
...
print('------------------------------------------------QUEUE METRICS-------------------------------------------------------')
for m in instance.m:
    q_path = [value(instance.Q[m,t]) for t in instance.t]
    print(f"MS site {m}: max queue = {max(q_path):.1f}, average queue = {sum(q_path)/len(q_path):.2f}")
print('--------------------------------------------------------------------------------------------------------------------')
