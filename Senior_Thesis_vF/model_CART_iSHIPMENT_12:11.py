...
# NEW: Binary variable for when manufacturing starts
model.Z = Var(model.p, model.m, model.t, within=Binary) # 1 if manufacturing of therapy p starts at site m at time t
...
model.INM = Var(model.p, model.m, model.t, within=NonNegativeReals) # Arrival at manufacturing site (into queue)
model.STARTM = Var(model.p, model.m, model.t, within=NonNegativeReals) # NEW: Start of manufacturing (out of queue)
...
# NEW: Queue variables
model.QUEUE = Var(model.p, model.m, model.t, within=NonNegativeReals) # Samples waiting in queue at site m at time t
model.QTIME = Var(model.p, within=NonNegativeReals) # Queue waiting time for therapy p
...
# NEW: Track arrival and manufacturing start times for queue calculation
model.ARR_TIME = Var(model.p, within=NonNegativeReals) # Time therapy p arrives at manufacturing
model.MFG_START_TIME = Var(model.p, within=NonNegativeReals) # Time therapy p starts manufacturing
...
# ============================================================================
# MODIFIED UTILIZATION CONSTRAINTS - Now based on STARTM instead of INM
# ============================================================================

#The percentage of utilisation of facility m at time t (based on when manufacturing STARTS, not arrival)
...
# DURV now tracks based on STARTM (manufacturing start) instead of INM (arrival)
def MSBnew_rule(model,p,m,t):
     return model.DURV[p,m,t] == sum(model.STARTM[p,m,tt-1]-model.OUTM[p,m,tt] for tt in model.tt if tt<=t and tt>1) + model.OUTM[p,m,t] 
model.MSBnew = Constraint(model.p, model.m, model.t, rule=MSBnew_rule)

# NEW: Maximum utilization constraint
...
#MSB1 - Leukapheresis timing (unchanged)
...
#MSB3 - Transport to manufacturing (unchanged - samples still arrive at INM)
...
#MSB7 - Output from leukapheresis (unchanged)
...
#MSB5 - Arrival at manufacturing site (unchanged - this is arrival into queue)
...
# ============================================================================
# NEW QUEUE CONSTRAINTS
# ============================================================================

# Queue balance: queue at t = queue at t-1 + arrivals - manufacturing starts
...
# Each therapy starts manufacturing exactly once at one site
...
# Link Z (binary) to STARTM (continuous) - STARTM = Z for this single-unit flow
...
# Manufacturing can only start at a site where the sample arrived
# Sum of STARTM over time must equal sum of INM over time (for each p,m pair)
...
# Manufacturing cannot start before arrival
# Cumulative starts <= Cumulative arrivals at each time point
def START_AFTER_ARRIVAL_rule(model, p, m, t):
    return sum(model.STARTM[p,m,tt] for tt in model.t if tt <= t) <= sum(model.INM[p,m,tt] for tt in model.t if tt <= t)
model.START_AFTER_ARRIVAL = Constraint(model.p, model.m, model.t, rule=START_AFTER_ARRIVAL_rule)

# Track arrival time for queue time calculation
...
# Track manufacturing start time
...
# Queue time = manufacturing start - arrival time
...
# ============================================================================
# MODIFIED: MSB2 - Manufacturing duration now based on STARTM
# ============================================================================
#MSB2 - Manufacturing takes TMFE days from START (not arrival)
def MSB2_rule(model,p,m,t,tt):
    if tt == t + model.TMFE:
        return model.STARTM[p,m,t] == model.OUTM[p,m,tt]
    else:
        return Constraint.Skip
model.MSB2 = Constraint(model.p, model.m, model.t, model.tt, rule=MSB2_rule)

#MSB8 - QC timing (unchanged - still based on OUTM)
...
#MSB4 - Transport to hospital (unchanged)
...
#MSB6 - Hospital arrival (unchanged)
...
# ============================================================================
# CAPACITY CONSTRAINTS - Now based on STARTM
# ============================================================================
def CAP1_rule(model,m,t):
    return model.CAP[m,t] == model.FCAP[m]-sum(model.STARTM[p,m,tt] for p in model.p for tt in model.tt if tt<t and tt>=t-model.TMFE)
...
# NEW: Force the model to use m1 and m4 specifically (to test queue mechanism)
# Comment out this constraint if you want the model to choose facilities freely
...
print('-----------------------------------------------------------------------------------------------------------------------')
for p in instance.p:
    if value(instance.QTIME[p]) > 1e-3:
        print('Therapy',p,'waited in queue for',value(instance.QTIME[p]),'days')
print('-----------------------------------------------------------------------------------------------------------------------')
print('Average queue time:', sum(value(instance.QTIME[p]) for p in instance.p)/len(instance.p), 'days')
print('Max queue time:', max(value(instance.QTIME[p]) for p in instance.p), 'days')
print('-----------------------------------------------------------------------------------------------------------------------')
    ...
    for m in instance.m:
        for t in instance.t:
            if value(instance.INM[p,m,t]) == 1:
                print('Therapy',p,'arriving at manufacturing site',m,'at time','t{}'.format(t))
print('-----------------------------------------------------------------------------------------------------------------------')
print('-----------------------------------------------------------------------------------------------------------------------')
print('-----------------------------------------------------------------------------------------------------------------------')    
...
for p in instance.p:
    for m in instance.m:
        for t in instance.t:
            if value(instance.DURV[p,m,t]) == 1:
                print('Therapy',p,'is manufactured at site',m,'for the time period','t{}'.format(t-model.TMFE),'-','t{}'.format(t))
print('-----------------------------------------------------------------------------------------------------------------------')
print('-----------------------------------------------------------------------------------------------------------------------')
print('-----------------------------------------------------------------------------------------------------------------------')
print('------------------------------------------------OUTM(p,m,t)------------------------------------------------------------')
